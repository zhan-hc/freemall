<template>
    <div>
        ...
        <!-- 面包屑 -->
  <nav-crumbs :crumbs="'地址'"></nav-crumbs>
  <!-- 地址确认 -->
    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li  :class="{'check':checkedIndex == index}" v-for="(item,index) in adrlistFilter" :key="item.addressId" @click="checkedIndex=index">
                <dl>
                  <dt>{{item.userName}}</dt>
...
import NavCrumbs from './../components/Breadcrumbs.vue'
export default {
    name: 'pages-address',
    data() {
        return{
          limit: 3,//显示地址的数量
          checkedIndex: 0,//点击地址的索引
          modalConfirm: false,
          delitem: '',
          adrlist: []
        }
    ...
    computed: {
      adrlistFilter(){
        return this.adrlist.slice(0,this.limit);
      }
    },
    mounted(){
      this.init();//初始化购物车列表数据
    },
    methods:{
      //初始化购物车列表数据
      init(){
      ...
      expand(){
        if(this.limit == 3){
          this.limit = this.adrlist.length;
        }
        else{
          this.limit = 3;
        }
      ...
      delAddress(){
        ...
        this.adrlist.map((item,index)=>{
          if(delItem.addressId == item.addressId){
            this.adrlist.splice(index,1);
